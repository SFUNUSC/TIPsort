	...
	if((cev->tg.h.AHP&(one<<(pos-1)))!=0)
	  {
	    ...
	    for(pin=1;pin<NPIN;pin++)
	      if(((cev->pinarray.h.THP&(one<<pin))!=0) && (take==0))
		{
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_PINArrayTCalLimABSuppLayerSum master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalABSuppLayerSum histograms for TIGRESS gated on PINArrayTCal.\n");
