...
int analyze_data(raw_event *data)
{
  ...
  /* 	if(cev->tg.det[pos].ge[col].suppress) */
  /* 	  { */
  /* 	    if(take==0) */
  /* 	      //if(cev->tg.det[pos].ge[col].suppress>=15200 && cev->tg.det[pos].ge[col].suppress<=16130 && take==0) */
  /* 	      {  */
  	      ...
  	      if((cev->tg.h.AHP&(1<<(pos-1)))!=0)
  		{
  		  ...
  		  for(col=0;col<NCOL;col++)
  			...
  			if(cev->tg.det[pos].ge[col].h.FH>0)
  			  ...
  			  if(cev->tg.det[pos].ge[col].suppress>=950 && cev->tg.det[pos].ge[col].suppress<=1010 && take==0)
  			    //if(cev->tg.det[pos].ge[col].suppress>=15200 && cev->tg.det[pos].ge[col].suppress<=16130 && take==0)
  			    {
  			      /* once suppression flag is set
  				 do not reset it, could remove the take bit
