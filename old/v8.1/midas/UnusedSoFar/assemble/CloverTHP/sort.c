...
int analyze_assembled_event(raw_event* data)
{
  ...
  for(det=1;det<NPOSTIGR;det++)
	...
	if((hp=data->tg.det[pos].ge[col].h.THP)!=0)
	  for(j=0;j<NHP;j++)
	    if((hp&(1<<j))!=0)
	      h[col]->Fill(j);
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=4)
    {
      printf("\n ./assemble_CloverTHP midas_input_data_file_name map Clover_position\n");
      exit(-1);
    }
...
/* do sorting */
 ...
 /* display results */
  printf("\n"); 
       ...
       printf("Program sorts time hit patterns for Tigress detectors \n");
