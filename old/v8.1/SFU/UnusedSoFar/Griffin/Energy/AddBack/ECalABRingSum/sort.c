...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Griffin_ECalABRingSum master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalABRingSum histograms for GRIFFIN.\n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting ECalABRingSum histograms for GRIFFIN clovers and cores based upon the cluster file: %s\n",name->fname.cluster_file);
