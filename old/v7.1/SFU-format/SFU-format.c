...
/*===============================================================*/
void display_channel_new(int i,channel* d,double t)
{
  printf(" %4d     %8.8x  %8.8x  %8.8x  %8.8x  %.8f\n",i,d->timestamp_up&0x00ffffff,d->timestamp&0x00ffffff,d->cfd,d->charge,t);  
}
/*===============================================================*/
...
{
  printf("Channel    TS_high   TS low     CFD      Charge   T0_from_fit\n");
}
/*===============================================================*/
void display_segment_legend()
{
  printf("Segment    TS_high   TS low     CFD      Charge   T0_from_fit\n");
}
/*===============================================================*/
...
{									
  ...
  for(int i=0;i<NPIN;i++)
    if((d->h.TSHP&(one<<i))!=0)	display_channel_new(i,&d->pin[i],d->t0[i]);
}
/*===============================================================*/
...
{									
  ...
  for(int i=0;i<NCSI;i++)
    if((d->h.TSHP&(one<<i))!=0) display_channel_new(i,&d->csi[i],d->t0[i]);
}
/*===============================================================*/
      ...
      if((d->h.GeHP&(1<<col))!=0)
	{
	  ...
	  for(seg=0;seg<NSEGTIGR;seg++)
	    if((d->ge[col].h.TSHP&(1<<seg))!=0)
	      {
		display_channel_new(seg,&d->ge[col].seg[seg],d->ge[col].t0[seg]);	    
	      }}
  if(&d->h.BGOfold>0)
    for(col=0;col<NCOL;col++)
