	...
	if(cev->tg.det[TIGpos].hge.FE>0)
	  ...
	  if((cev->tg.h.AHP&(1<<(TIGpos-1)))!=0)
	      ...
	      for(TIGcol=0;TIGcol<NCOL;TIGcol++)
		{
	      ...
	      eAddBack = cev->tg.det[TIGpos].addback.E/cal_par->tg.contr_e;  
	      addBackCol = cev->tg.det[TIGpos].addbackC;
	      addBackCol += NCOL*suppFlag;
	      
	      
	      if(eAddBack>=0 && eAddBack<=S32K)
		hist[TIGpos][addBackCol][(int)rint(eAddBack)]++;
	      else
...
int main(int argc, char *argv[])
{
  ...
  for(pos=1;pos<NPOSTIGR;pos++)
    {
      sprintf(mcaFile,"pos%1d%1d_ECalABSuppSumBGOTLimits.mca",pos/10,pos-(pos/10)*10);
