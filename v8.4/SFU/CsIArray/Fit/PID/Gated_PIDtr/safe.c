...
int main(int argc, char *argv[])
{
  ...
  TFile *m,*n;
  FILE *gateNameFile;
  char aGateName[132],pGateName[132],HistName[132];
  int detector;
  ...
  if(name->flag.gate_name_file==1)
	...
	{
	  printf("WHOO!");
	  fscanf(gateNameFile,"%d %s %s",detector,aGateName,pGateName);



	  printf("The gates for pos %d are %s and %s\n",pos,aGateName,pGateName);
	  if(pos=detector)
	    {
	      aGate[pos] = (TCutG *) gROOT->FindObject(aGateName);
	      pGate[pos] = (TCutG *) gROOT->FindObject(pGateName);
	    }
	  else printf("Position %d has no cuts.\n",pos);
	}
      fclose(gateNameFile);
    }
  else
