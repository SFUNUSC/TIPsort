...
int analyze_data(raw_event *data)
{
  ...
  cal_event* cev;
  double tAddBack=0,eAddBack=0,tPIN=0,tForGating=0;
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_PINArrayRingsECalABSuppLayerSum_AllTLim master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalABSuppLayerSum histograms for TIGRESS gated on PINArrayRings and all TLimits.\n");
