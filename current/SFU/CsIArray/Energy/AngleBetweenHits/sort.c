...
int analyze_data(raw_event *data) {
  ...
  if (csiHitIndex >= 2) // are we able to get an angle (have 2 hits?)
  {
    ...
    for (int i = 0; i < 2; i++)
      res_dir_mag[i] =
          sqrt(res_dir[i][0] * res_dir[i][0] + res_dir[i][1] * res_dir[i][1] +
               res_dir[i][2] * res_dir[i][2]);
    for (int i = 0; i < 3; i++)
...
int main(int argc, char *argv[]) {
  ...
  positionHist = new TH1D("CosAngleHistogramFromPositions",
                          "CosAngleHistogramFromPositions", 200, -1, 1);
  positionHist->Reset();
  h = new TH1D("CosAngleHistogramFromData", "CosAngleHistogramFromData", 200,
               -1, 1);
  h->Reset();
  ...
  canvas = new TCanvas("AngleHistogramFromData", "AngleHistogramFromData", 10,
                       10, 500, 300);
  gPad->SetLogy(0);
  h->Draw();
  theApp->Run(kTRUE);
  ...
  theApp = new TApplication("App", &argc, argv);
  canvas = new TCanvas("AngleHistogramFromPositions",
                       "AngleHistogramFromPositions", 10, 10, 500, 300);
  gPad->SetLogy(0);
  positionHist->Draw();
  theApp->Run(kTRUE);
}
