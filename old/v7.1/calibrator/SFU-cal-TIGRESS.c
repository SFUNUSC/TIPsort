...
void read_TIGRESS_core_time_limits(TIGRESS_calibration_parameters *TIGRESS_cal_par, char *filename)
{
  ...
  printf("TIGRESS time limits read from %s.\n",filename);
		...
		if(raw_event->tg.det[pos].ge[col].h.Efold>0)
		  if((raw_event->tg.det[pos].ge[col].h.EHP&1)!=0)
		      ...
		      if((e>=0) && (e<4*S32K))
			{
		      ...
		      else if(e<0) printf("Warning! Calibrated Tigress energy is less than 0!\n");
		      else         printf("Warning! Calibrated Tigress energy is greater than 4*S23K! Raw energy was %f.\n",(ren-ran));
		    }	
		  ...
		  if((TIGRESS_cal_ev->det[pos].ge[col].h.EHP&1)!=0)
		    {
		      ...
		      //The next lines are absolutely needed to make sure that
		      //the cores used for add back were from gammas in the proper
		      //beam bunch. If the data has been 2D separated, then this is
		      //not a necessary step, since all fragments not in the right
		      //bunch are thrown out of the data stream....
		      
		      //if((t>TIGRESS_cal_par->ctlow[pos][col]) &&
		      //	(t<TIGRESS_cal_par->cthigh[pos][col]))
		      //{
  ...
  for(pos=1;pos<NPOSTIGR;pos++)
    {
      ...
      if(raw_event->tg.h.BGOfold>0) /* BGO in the event. ANYWHERE.*/
	/* BGO at the right position in the event */
	if(((raw_event->tg.h.BGOHP&(one<<(pos-1)))!=0) && (raw_event->tg.det[pos].h.BGOfold>0))
