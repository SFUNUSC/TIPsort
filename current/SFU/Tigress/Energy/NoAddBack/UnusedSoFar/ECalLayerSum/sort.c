  	...
  	if(cev->tg.det[pos].hge.FE>0)
  	  for(col=0;col<NCOL;col++)
  		...
  		if((cev->tg.det[pos].ge[col].h.EHP&1)!=0)
		    ...
		    if((eCal>0) && (eCal<S32K))
		      {
			if((ring>0) && (ring<NRING)) hist[layer][(int)rint(eCal)]++;
		      }
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_ECalLayerSum master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalLayerSum histograms for TIGRESS.\n");
  ...
  if((output=fopen("Layer_ECalSum.mca","w"))==NULL)
    {
