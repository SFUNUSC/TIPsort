...
int analyze_data(raw_event *data)
{
  ...
  //check energy difference from CsI array calibration
  u=cev->csiarray.U;
  //printf("u: %f \n",u);
  //getc(stdin);
  if(u>=18000)
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("TigressCsI_ECalABSuppRingSum_deltaUcut master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalABSuppRingSum histograms for TIGRESS with deltaU cuts imposed by the CsI array.\n");
  ...
  if((output=fopen("Ring_ECalABSuppSum_deltaUcut.mca","w"))==NULL)
    {
