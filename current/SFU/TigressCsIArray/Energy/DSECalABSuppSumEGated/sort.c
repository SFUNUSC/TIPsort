                ...
                if(eAddBack>=0 && eAddBack<S32K)
                  {
                    ...
                    if(eAddBack>=0 && eAddBack<S32K)
                    	projhist[ring[i]][(int)energyds[i]]++;
				              ...
				              if(j!=i)
				                if(energyds[j]>=0)
				                  if(energyds[j]<S32K)
				                    if(ring[j]>0)
				                      if(ring[j]<NRING)
				                        hist[ring[j]][(int)(energyds[j])]++;
		                }
		              gatehist[ring[i]][(int)(energyds[i])]++;
...
int main(int argc, char *argv[])
{
  ...
  if((argc!=6)&&(argc!=7))
    {
      printf("TigressCsIArray_DSECalABSuppSumEGated master_file_name supLow supHigh gateELow gateEHigh fudge_factor\n");
      printf("Program attempts to generate energy gated gamma ray spectra where the energy gate is applied to the original Doppler shifted data, but the gated spectrum written to disk is Doppler unshifted.  Energy gate values (gateEHigh, gateELow) should be specified in keV.\n");
  ...
  printf("Program attempts to generate energy gated gamma ray spectra where the energy gate is applied to the original Doppler shifted data, but the gated spectrum written to disk is Doppler unshifted.\n");
  ...
  sprintf(FileName,"DSECalABSuppSum_c%.0f_w%.0fgated.mca",avgGateE,gateWidth);
  ...
  sprintf(FileName,"DSECalABSuppSum_c%.0f_w%.0fgate.mca",avgGateE,gateWidth);
  if((output=fopen(FileName,"w"))==NULL)
    {
      printf("ERROR!!! I cannot open the mca file!\n");
      exit(EXIT_FAILURE);
  ...
  sprintf(FileName,"DSECalABSuppSum_c%.0f_w%.0fproj.mca",avgGateE,gateWidth);
  if((output=fopen(FileName,"w"))==NULL)
    {
      printf("ERROR!!! I cannot open the mca file!\n");
      exit(EXIT_FAILURE);
