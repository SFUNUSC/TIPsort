   ...
   while( evntbuf < bankend ){
	...
	break;
      case 0x5:                             /*        Charge */
         ...
         ptr->charge = value & 0x03FFFFFF;
	 ptr->charge_flag++;
         if( value & 0x04000000 ){      /* pileup bit of charge */
            ptr->pileup_flag++;
	 }
         if( value & 0x08000000 ){      /* over range bit of charge */
            ptr->overange_flag++;
         }
         break;
      case 0x6:                             /*      CFD Time */
	...
	fprintf(stderr,"Unknown type %4.4x\n",type);
	print_fragment_info(ptr,0);
