...
// checks whether a value falls within the range of two bounds (lower bound
// inclusive)
              ...
              if ((cev->tg.det[pos].hge.HHP & (1 << col)) != 0)
                ...
                if (cev->tg.det[pos].ge[col].h.FH > 0)
                  ...
                  if ((cev->tg.det[pos].ge[col].h.HHP & 1) != 0)
                    // suppress if the position is in the map and has not yet
                    // been suppressed
                    if (cev->tg.det[pos].ge[col].suppress >= supLow &&
                        cev->tg.det[pos].ge[col].suppress <= supHigh &&
                        take == 0) {
            ...
            if (eAddBack >= 0 && eAddBack < S32K) {
              if (cev->csiarray.h.FH > 0)
                for (csi = 1; csi < NCSI; csi++)
                  if ((cev->csiarray.h.HHP[csi / 64] & (one << csi % 64)) !=
                      0) {
                    ...
                    for (int ind = 0; ind < 3; ind++)
                      part_dir[csi][ind] = part_dir[csi][ind] /
              ...
              pval = sqrt(res_p[0] * res_p[0] + res_p[1] * res_p[1] +
                          res_p[2] * res_p[2]);
              beta =
                  sqrt(1.0 - ((cal_par->csiarray.mr * cal_par->csiarray.mr) /
                              ((pval * pval) +
                               (cal_par->csiarray.mr *
                                cal_par->csiarray
...
int main(int argc, char *argv[]) {
  ...
  if ((argc != 4) && (argc != 5)) {
    printf("TigressCsI_ECalABSuppDSReconstructed master_file_name supLow "
           "supHigh fudge_factor\n");
    printf("Program attempts to generate gamma ray spectra with all events "
           "Doppler unshifted.  Doesn't work for transitions with lifetimes "
           "long enough for the residual nucleus to slow.\n");
    printf("Relies on beam momentum and velocity values specified in "
           "calibration parameters (deltaU.par).\n");
