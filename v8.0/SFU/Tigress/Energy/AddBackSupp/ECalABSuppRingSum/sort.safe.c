	  ...
	  if((cev->tg.h.AHP&(1<<(pos-1)))!=0)
	    {
	      ...
	      suppFlag = cev->tg.det[pos].ge[col].suppress; //For suppression map
	      //suppFlag = cev->tg.det[pos].suppress; //For default suppression
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_ECalABSuppRingSum master_file_name\n");
      exit(-1);
    }
