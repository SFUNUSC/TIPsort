...
int main(int argc, char *argv[])
{
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting ECalSum histograms for CsIArray based upon the cluster file: %s\n",name->fname.cluster_file);
  ...
  while(fscanf(cluster,"%s",DataFile) != EOF)
    {
      ...
      printf("Sorting CsI cal. energy data from file %s\n", name);
      sort(name);
    }
  
  if((output=fopen("CsIArray_ECalSum.spn","w"))==NULL)
    {
