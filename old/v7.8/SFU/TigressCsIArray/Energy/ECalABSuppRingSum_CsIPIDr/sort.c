...
int analyze_data(raw_event *data)
{
  ...
  for(pos=1;pos<NCSI;pos++)
	...
	if((aGateLowFlag[pos]==1)&&(aGateHighFlag[pos]==1)&&(pGateHighFlag[pos]==1))
	  {
	      ...
	      if((chi>=chimin) && (chi<=chimax))
		{
		  ...
		  if((aGateHigh[pos]<r) && (r<=pGateHigh[pos]))
		    { 
		      np++;
		       /* if(pos==9) */
		       /* 	pos9Flag=1; */
		       //printf("proton in pos %d     np = %d\n",pos,np);
		    }
		  if((aGateLow[pos]<=r) && (r<=aGateHigh[pos]))
		    {
		      na++;
		        /* if(pos==9) */
		      	/* pos9Flag=1; */
			//printf("alpha in pos %d     na = %d\n",pos,np);
		    }
		}
	  }
	else
  ...
  for(int m=0;m<NSP;m++)
    {
      ...
      if(m==7) sprintf(evaporationChannel,"1a2p");
      if(m==8) sprintf(evaporationChannel,"3a");
      if(m==9) sprintf(evaporationChannel,"3p");
      if(m==10) sprintf(evaporationChannel,"2a1p");
