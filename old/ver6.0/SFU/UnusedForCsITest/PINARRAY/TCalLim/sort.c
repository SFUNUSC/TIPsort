...
int analyze_data(raw_event *data)
{
  ...
  if(cev->pinarray.h.FT>0)
      ...
      if((cev->pinarray.h.THP&(one<<pos))!=0)
	{
	  ...
	  if(t>cal_par->pinarray.tlow[pos])
	    if(t<cal_par->pinarray.thigh[pos])
	      {
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("\n ./PINARRAY_TCalLim master_file_name\n");
      exit(-1);
    }
  
  h = new TH2D("Raw Time","Raw Time",S4K,0,S4K-1,NPIN+1,0,NPIN);
  h->Reset();
  ...
  if((output=fopen("PINArray_cal_time.spn","w"))==NULL)
    {
      printf("\nI can't open file %s\n","PINArray_cal_time.spn");
      exit(EXIT_FAILURE);
    }
