                ...
                if(eAddBack>=ELow && eAddBack<=EHigh)
                  {
                    ...
                    //calculate theta
                    theta=acos(res_dir[0]*gamma_dir[0] + res_dir[1]*gamma_dir[1] + res_dir[2]*gamma_dir[2])*180./PI;
                    
                    if(suppFlag==0)
                    	{
                    		h->Fill(theta,eAddBack);
                    		sprintf(str,"%lf %lf\n",theta,eAddBack);
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=6)
    {
      printf("TigressCsI_EThetaCalABSuppSum master_file_name suppLow suppHigh ELow EHigh\n");
  ...
  h = new TH2D("EThetaHistogram","EThetaHistogram",180,0,180,EHigh-ELow,ELow,EHigh);
  h->Reset();
  
  printf("Program sorts EThetaCalABSuppSum histograms for TIGRESS.\n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting EThetaCalABSuppSum histograms for TIGRESS clovers and cores based upon the cluster file: %s\n",name->fname.cluster_file);
