...
int analyze_data(raw_event *data)
{
  ...
  if((cev->tg.h.FH>0) && (cev->pinarray.h.FH>0))
      ...
      if((cev->pinarray.h.HHP&(one<<pin))!=0)
	{
	  ...
	  ring = (cal_par->pinarray.ring_map[pin] - 1);
	  take=0;
	  ...
	  for(pos=1;pos<NPOSTIGR;pos++)
	      ...
	      if((cev->tg.h.AHP&(one<<(pos-1)))!=0)
		{
		  ...
		  if((eAddBack>cal_par->tg.celow[pos][col]) && (eAddBack<cal_par->tg.cehigh[pos][col]) && (suppFlag==0))
		    {
		      ...
		      if((ePIN>0) && (ePIN<S32K)) histPIN[ring][(int)rint(ePIN)]++;
		      else histPIN[ring][S32K-1000]++;
		    }}}
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("PINArrayRing_TigECalLimSum master_file_name\n");
      exit(-1);
    }
  
  h=new TH2D("PINRing_TigECalLimSum","PINRing_TigECalLimSum",S8K,0,S32K-8,NPINRING+1,0,NPINRING);
  h->Reset();
  
  printf("Program sorts PINArrayRing_ECalSum spectra gated on TigECalLim.\n");
  ...
  if((output=fopen("PINArrayRing_TigECalLimSum.mca","w"))==NULL)
    {
      printf("ERROR!!! I cannot open the mca file!\n");
      exit(EXIT_FAILURE);
    }
  
  fwrite(histPIN,NPINRING*S32K*sizeof(int),1,output);
  fclose(output);
