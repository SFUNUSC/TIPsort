	...
	if((cev->tg.h.AHP&(1<<(pos1-1)))!=0)
	  {
		...
		if((cev->tg.det[pos1].hge.HHP&(1<<col1))!=0)
		  ...
		  //suppress if the position is in the map and has not yet been suppressed
		    if(cev->tg.det[pos1].ge[col1].suppress>=940 && cev->tg.det[pos1].ge[col1].suppress<=1060 && take1==0)
		      {
	  ...
	  for(pos2=pos1+1;pos2<NPOSTIGR;pos2++)
	      ...
	      if((cev->tg.h.AHP&(1<<(pos2-1)))!=0)
		{
		  ...
		  for(col2=0;col2<NCOL;col2++)
			...
			//suppress if the position is in the map and has not yet been suppressed
			  if(cev->tg.det[pos2].ge[col2].suppress>=940 && cev->tg.det[pos2].ge[col2].suppress<=1060 && take2==0)
			    {
			      /* once suppression flag is set
				 do not reset it, could remove the take bit
				 and keep resetting suppFlag, but this
		  ...
		  /* CONDITIONS FOR MAKING THE MATRIX */
		  //if the events occur at the proper positions
		  //if(pos1==4)
		  //if(pos2==16)
		  //{
			//printf("after check\n pos1 %d pos2 %d\n",pos1,pos2);
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("\n ./TIGRESS_EECalABSuppSum master_file_name\n");
      exit(-1);
    }
