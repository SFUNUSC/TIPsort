...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("\n ./GRIFFIN_TECal master_file_name\n");
      exit(-1);
    }
  
  h = new TH2D("TTCal","TTCal",S1K,512,S1K,S8K,0,S8K);
  h->Reset();

  h_t = new TH1D("TTCal_t","TTCal_t",S1K,512,S1K);
  h_t->Reset();
  h_e = new TH1D("TTCal_e","TTCal_e",S8K,0,S8K);
  h_e->Reset();
