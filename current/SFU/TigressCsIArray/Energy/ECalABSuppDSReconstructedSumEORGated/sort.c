  ...
  for(i=0;i<cev->tg.h.FA;i++)
    {
      ...
      if((energy[i]>=0)&&(energy[i]<S32K))
        if(((energy[i]>=gateELow/cal_par->tg.contr_e)&&(energy[i]<=gateEHigh/cal_par->tg.contr_e)) || ((energy[i]>=gate2ELow/cal_par->tg.contr_e)&&(energy[i]<=gate2EHigh/cal_par->tg.contr_e)))
	          if(ring[i]>0)
	          	if(ring[i]<NRING)
		            {
...
int main(int argc, char *argv[])
{
  ...
  if((argc!=8)&&(argc!=9))
    {
      printf("TigressCsI_ECalABSuppDSReconstructedSumEORGated master_file_name supLow supHigh gate1ELow gate1EHigh gate2ELow gate2EHigh fudge_factor\n");
  ...
  gate2ELow = atof(argv[6]);
  gate2EHigh = atof(argv[7]);
  if(argc==9)
