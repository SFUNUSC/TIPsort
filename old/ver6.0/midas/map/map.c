	  ...
	  if(pos<NPOSTIGR)
	      ...
	      if(type==1)
		{
		  ...
		  if(ptr->charge_flag==1)
		    if(ptr->charge!=0)
		      {
			data->tg.det[pos].ge[col].seg[d].charge=ptr->charge;
	  ...
	  if(pos<NPOSGRIF)
	    {
		  ...
		  if(waveform!=NULL)
			...
			get_Griffin_t0(ptr->waveform_length,waveform,&wpar);
		       	data->gr.det[pos].ge[col].t0[d]=wpar.t00;
			take|=1;
		      }
	  ...
	  if(pos<NCSI)
	      ...
	      if(take==1)
		{
		  ...
		  if(waveform!=NULL)
		    if(map->csiarray_fit==1)
		      {
			fit_waveform(ptr->waveform_length,waveform,&data->csiarray.wfit[pos],&wpar);
			get_CsI_t0(ptr->waveform_length,waveform,&data->csiarray.wfit[pos],&wpar);
			data->csiarray.t0[pos]=wpar.t00;
		      }
		}
	    }
      }
