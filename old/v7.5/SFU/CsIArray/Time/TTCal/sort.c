	  ...
	  if((cev->csiarray.h.THP&(one<<pos2))!=0)
	    {
	      ...
	      //h_t1 is plot of time of detector that fired
	      //first. This will have a smaller value.
	      if(t1>t2)
		{
		  tdiff=t1-t2;
		  h_t1->Fill(t2);
...
int main(int argc, char *argv[])
{
  ...
  h = new TH2D("CsI TTCal","CsI TTCal",S8K,0,S32K-1,S8K,0,S32K-1);
  h->Reset();
  h_t1 = new TH1D("TCal1","TCal1",S8K,0,S32K-1);
  h_t1->Reset();
  h_t2 = new TH1D("TCal2","TCal2",S8K,0,S32K-1);
  h_t2->Reset();
  h_tdiff = new TH1D("TCal Diff","TCal Diff",S8K,0,S32K-1);
  h_tdiff->Reset();
  ...
  sprintf("CsI TTCal","CsIArray_TTCal.root");
  TFile f("CsI TTCal", "recreate");
