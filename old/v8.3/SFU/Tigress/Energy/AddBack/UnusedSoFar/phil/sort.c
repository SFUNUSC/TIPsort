  ...
  if(cev->tg.h.FH>0)
    for(pos=1;pos<NPOSTIGR;pos++)
	  ...
	  {	
	    ...
	    //printf("Und el triggero numero esta es %d\n",data->h.trig_num&0x7fffffff);
	    if((eAddBack>=0) && (eAddBack<S32K))
	      {
		if((ring>0) && (ring<NRING)) 
		  {
		    for(int i=0; i<4; i++)
		      if(abs(eAddBack-antiGate[i])<(10/cal_par->tg.contr_e))
			{
			  //printf("We out! This was for position %d\n",pos);
			  //getc(stdin);
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_ECalABRingSum master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalABRingSum histograms for TIGRESS.\n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting ECalABRingSum histograms for TIGRESS clovers and cores based upon the cluster file: %s\n",name->fname.cluster_file);
  ...
  if((output=fopen("Ring_ECalABSum.mca","w"))==NULL)
    {
