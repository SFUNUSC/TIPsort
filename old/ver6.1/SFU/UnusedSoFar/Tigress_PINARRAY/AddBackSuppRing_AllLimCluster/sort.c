...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("\n ./Tigress_PINARRAY_AddBackSuppRing_AllLimCluster master_file_name\n");
      exit(-1);
    }
  

  printf("Program sorts calibrated TIGRESS ring suppressed addback spectra gated on PINARRAY and TIGRESS energies and times \n");
  ...
  if((output=fopen("gated_rings.mca","w"))==NULL)
    {
      printf("\nI can't open file ring_cal_core_energy.mca\n");
      exit(EXIT_FAILURE);
    }
