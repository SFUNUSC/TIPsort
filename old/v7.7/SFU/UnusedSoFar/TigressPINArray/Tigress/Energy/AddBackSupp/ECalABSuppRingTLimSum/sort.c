	...
	if((cev->tg.h.AHP&(one<<(pos-1)))!=0)
	  {
	    ...
	    //The calibrator for addBack makes sure cores in add back have
	    //a prompt time (defined by time limits...). However, I'll check 
	    //here that the core with the highest energy deposit (the "first"
	    //interaction location) had time within the gate. Just to check.

	    tForGating = cev->tg.det[pos].ge[col].seg[0].T;
	    if((tForGating<cal_par->tg.ctlow[pos][col]) ||
	       (tForGating>cal_par->tg.cthigh[pos][col]))
	      {
		printf("Someting isn't right here...this core's time shoulda been ok!\n");
		getc(stdin);
	      }
	      ...
	      if((cev->pinarray.h.HHP&(one<<pin))!=0)
		{
		  ...
		  if((tPIN>cal_par->pinarray.tlow[pin]) && (tPIN<cal_par->pinarray.thigh[pin]))
		    {
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_ECalABSuppRingSum_AllTLim master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts TIGRESS ECalABSuppRing spectra gated on PINArray and TIGRESS times.\n");
  ...
  if((output=fopen("Ring_AllTLimABSuppSum.mca","w"))==NULL)
    {
