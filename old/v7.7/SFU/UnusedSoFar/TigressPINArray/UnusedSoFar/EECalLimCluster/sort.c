...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("\n ./TIGRESS_Pinarray_EECalCluster master_file_name\n");
      exit(-1);
    }
  
  h = new TH2D("Tigress Pinarray Energy","Tigress Pinarray Energy",S4K,0,S4K-1,S4K,0,S4K-1);
  h->Reset();

  printf("Program sorts calibrated 2D histogram for TIGRESS/PINARRAY energy \n");
