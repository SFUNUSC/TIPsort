...
int main(int argc, char *argv[])
{
  ...
  if(argc!=6)
    {
      printf("Tigress_ECalABRingSum_CsIPIDr master_file_name idmin idmax chimin chimax\n");
      exit(-1);
    }
  
  printf("Program generates Tigress ECalABRing histograms separated on CsIPIDr.\n");
  ...
  for(int m=0;m<NSP;m++)
    {
      ...
      sprintf(fileName,"ECalABRingSum_%s_Gated.mca",evaporationChannel);
