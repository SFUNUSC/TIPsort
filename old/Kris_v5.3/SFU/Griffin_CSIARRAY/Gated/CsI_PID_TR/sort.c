...
int analyze_data(raw_event *data)
{
  ...
  //first we check if all the elements of the setup are present
  //and discard the event if not
  ...
  //next we work out number of particles identified in the event
  ...
  //next we check if there is proton or alpha
  //if not we discard the event
  if((pGateFlag==0)&&(aGateFlag)==0)
    return SEPARATOR_DISCARD;

  //since we have now an identified particle in proper coincidence
  //we calibrate GRIFFIN (earlier is just a waste of CPU)
  cev=(cal_event*)malloc(sizeof(cal_event));
  memset(cev,0,sizeof(cal_event));
  calibrate_GRIFFIN(data,&cal_par->gr,&cev->gr);

  //next we sort particle gated spectra
  if(cev->gr.h.FE>0)
		...
		if((cev->gr.det[pos].ge[col].h.EHP&one)!=0)
		  {
		    ...
		    //use explitic conditions, the ones which you used before
		    //were not exclusive enough
		    //here comes the ap gate
		    if((aGateFlag==1)&&(pGateFlag==1)) 
		      ap_hist[pos][col][(int)rint(e)]++;
		    //here comes the a gate
		    if((aGateFlag==1)&&(pGateFlag==0)) 
		      a_hist[pos][col][(int)rint(e)]++;
		    //here comes the p gate
		    if((aGateFlag==0)&&(pGateFlag==1))
		       p_hist[pos][col][(int)rint(e)]++;	
		  }
  free(cev);
  return SEPARATOR_DISCARD;
}
...
int main(int argc, char *argv[])
{
  ...
  FILE *output1,*output2,*output3,*gateNameFile;
  TFile *f;
  int stop;
  char a[132],p[132],ap[132];
  char aGateName[132],pGateName[132];
  ...
  if(name->flag.gate_name_file==1)
    {
      ...
      for(pos=1;pos<5;pos++)
	{
	  fscanf(gateNameFile,"%s %s",aGateName,pGateName);
