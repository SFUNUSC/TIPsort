            ...
            if (cev->csiarray.h.FH > 0)
              for (csi = 1; csi < NCSI; csi++)
                if ((cev->csiarray.h.HHP[csi / 64] & (one << csi % 64)) != 0) {
                  ...
                  for (int ind = 0; ind < 3; ind++)
                    part_dir[csi][ind] =
                        part_dir[csi][ind] / vecMag; // make vector unit length

                  ecsi =
                      cev->csiarray.csi[csi].E / 1000.; /* CsI energy in MeV */
                  pval = sqrt(
...
int main(int argc, char *argv[]) {
  ...
  if ((argc != 5) && (argc != 6)) {
    printf("TigressCsIArray_EECalABSuppDSReconstructedSum master_file_name "
           "supLow supHigh energy_contraction fudge_factor\n\n");
  ...
  h->Write();
	...
	printf("Output ROOT file: TigressCsIArray_EECalABSuppDSReconstructedSum.root\n");
}
