		  ...
		  if((data->tg.det[pos].ge[col].h.THP&1)!=0)
		    {
		      ...
		      if(((data->h.setupHP&RF_BIT)!=0))//&&data->h.setupHP!=52)
			{
			  trf=data->rf.sin.t0;
...
int main(int argc, char *argv[])
{
  ...
  char n[132], DataFileEntry[256];
  TApplication *theApp;

  if(argc!=2)
    {
      printf("Tigress_TRawSum master_file_name\n");
      exit(-1);
    }
  ...
  printf("Program sorts RF unwrapped TRawSum histograms for Tigress cores and crystals.\n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting unwrapped TIGRESS TRaw histograms based upon the cluster file: %s\n",name->fname.cluster_file);
  ...
  while(fscanf(ClusterFile,"%s",DataFileEntry) != EOF)
      ...
      if((DataFile=fopen(DataFileEntry,"r")) == NULL)
	{
      ...
      printf("Sorting RF unwrapped data from file %s\n", DataFileEntry);
      sort(name);
    }
  
  for(pos=1;pos<NPOSTIGR;pos++)
    {
      sprintf(n,"pos%1d%1d_TRawCoreCluster.mca",pos/10,pos-(pos/10)*10);
  ...
  printf("The event-dropper1 increments are %d.\n",count_1);
}
