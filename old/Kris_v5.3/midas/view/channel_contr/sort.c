...
int analyze_fragment(Tig10_event* ptr, short* waveform)
{
  ...
  if(ptr->channel==chn)
	...
	{
	  if(h!=NULL) delete h;
	  h=new TH1D("Waveform","Waveform",ptr->waveform_length/contr,0,ptr->waveform_length/contr);
	  if(c!=NULL) delete c;
	  c = new TCanvas("Waveform", "Waveform",10,10, 700, 500);
            
	  for(Int_t i=0;i<ptr->waveform_length/contr;i++)
	    for(Int_t j=0;j<contr;j++)
	      h->Fill(i,waveform[contr*i+j]/contr);
...
int main(int argc, char *argv[])
{
 ...
 if(argc!=4)
    {
      printf("\n ./view_channel midas_input_data_file_name channel contraction_factor\n");
      exit(-1);
    }
  chn=atoi(argv[2]);
  contr=atoi(argv[3]);
