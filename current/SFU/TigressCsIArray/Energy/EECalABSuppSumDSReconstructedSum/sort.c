	    ...
	    if((cev->tg.h.AHP&(1<<(pos1-1)))!=0)
	      {
					...
					eAddBack = cev->tg.det[pos1].addback.E/cal_par->tg.contr_e;  
          ...
          eAddBack=eAddBack/ds;
					...
					memset(part_p,0,sizeof(part_p)); //reset particle momenta for the next calculation
  				memset(gamma_dir,0,sizeof(gamma_dir));
									...
									if((cev->tg.h.AHP&(1<<(pos2-1)))!=0)
										{
											...
											eAddBack = cev->tg.det[pos2].addback.E/cal_par->tg.contr_e;  
											...
											//construct unit vectors for gamma and residual (source) nucleus
											for(int ind=0;ind<3;ind++)
												gamma_dir[ind]=cal_par->tg.tpos_xyz[pos2][colAddBack][ind];
											vecMag=sqrt(gamma_dir[0]*gamma_dir[0] + gamma_dir[1]*gamma_dir[1] + gamma_dir[2]*gamma_dir[2]);
											for(int ind=0;ind<3;ind++)
												gamma_dir[ind]=gamma_dir[ind]/vecMag;//make unit vector for gamma ray
											vecMag=sqrt(res_p[0]*res_p[0] + res_p[1]*res_p[1] + res_p[2]*res_p[2]);
...
int main(int argc, char *argv[])
{
  ...
  if((argc!=5)&&(argc!=6))
    {
      printf("TigressCsIArray_EECalABSuppDSReconstructedSum master_file_name supLow supHigh energy_contraction fudge_factor\n");
  ...
  sprintf(title,"TigressCsIArray_EECalABSuppDSReconstructedSum.root");
