...
int analyze_data(raw_event *data)
{
  ...
  for(csi=1;csi<NCSI;csi++)
	...
	if(type>=idmin && type<=idmax)
	  if(chi>=chimin && chi<=chimax) flag|=(one<<csi);
      }
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=7)
    {
      printf("Sep_CsI_FitType input_file_name output_file_name idmin idmax chimin chimax\n");
      exit(-1);
    }
  ...
  if((output=fopen(name->fname.out_data,"w"))==NULL)
    {
      printf("ERROR!!! I can't open output file!\n");
      exit(-2);
    }
