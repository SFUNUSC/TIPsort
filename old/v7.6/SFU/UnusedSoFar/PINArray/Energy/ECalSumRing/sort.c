...
int analyze_data(raw_event *data)
{
  ...
  if(cev->pinarray.h.FE>0)
      ...
      if((cev->pinarray.h.EHP&(one<<pos))!=0)
	{
	  ...
	  ring = (cal_par->pinarray.ring_map[pos] - 1);
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("PINArray_ECalSumRing master_file_name\n");
      exit(-1);
    }
  
  h = new TH2D("ECalSumRing","ECalSumRing",S8K,0,S32K-8,NPINRING+1,0,NPINRING);
  h->Reset();

  printf("Program sorts ECalSumRing histograms for the PINArray.\n");
  ...
  if((output=fopen("PINArray_ECalSumRing.mca","w"))==NULL)
    {
      printf("ERROR!!! I cannot open the mca file!\n");
      exit(EXIT_FAILURE);
    }

  fwrite(hist,NPINRING*S32K*sizeof(int),1,output);
  fclose(output);
  
  theApp=new TApplication("App", &argc, argv);
  canvas = new TCanvas("ECalSumRing", "ECalSumRing",10,10, 500, 300);
  gPad->SetLogz(1);
  gStyle->SetPalette(1);
  h->Draw("COLZ");
