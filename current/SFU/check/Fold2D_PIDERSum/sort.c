...
int main(int argc, char *argv[])
{
  ...
  FILE *gateNameFile, *cluster;
  TFile* f;
  TCanvas *canvas;
  TApplication *theApp;
  char title[132],DataFile[132],aGateName[132],pGateName[132],det[132];


  if(argc!=2)
    {
      printf("check_Fold2D_PIDERSum master_file\n");
      printf("Displays 2D fold histogram summed over multiple runs for protons and alphas whose gates at described in the gate_name_file specified in the master file.\n");
      printf("The master file should point to a cluster file containing the run names.\n");
      exit(-1);
    }
