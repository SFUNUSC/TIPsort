...
void assemble_event(int ind,node* list, data_pointers* dp,tmap* map)
{
  ...
  if(list[ind].next!=NULL)
    {
      //if((ind%100)==0) printf("assembling event %d\r",ind);
...
{ 
  int diff,i;
  int turn;
  //if((dp->proc%10000)==0) printf("Number of fragments added to the list is %8ld\r",dp->proc);
...
void sort_and_assemble(char* inp_name,char* map_name)
{
 ...
 /* get the map */
  ...
  while(stop==0){
    ...
    case END_OF_FILE:
      fclose(input);
      //printf("Number of analyzed fragments is %8ld\n",dp->proc);
      //printf("\n");
