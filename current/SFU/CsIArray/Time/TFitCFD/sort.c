...
int main(int argc, char *argv[])
{
  ...
  if(argc!=6)
    {
      printf("CsIArray_TFitCFD sfu_input_data alpha xbar chimin chimax\n x > xbar are alpha particles (more fast component)\n");
      exit(-1);
    }
  //histogram of CFD fit vs x=AF/(AF+AS)
  h = new TH2D("TFitCFD","TFitCFD",100,0,2,S4K,0,S4K-1);
  h->Reset();

  //talpha projection
  talpha_proj = new TH1D("Talpha","Talpha",S4K,0,S4K-1);
  talpha_proj->Reset();

  //tproton projection
  tproton_proj = new TH1D("Tproton","Tproton",S4K,0,S4K-1);
  tproton_proj->Reset();
  ...
  h->GetYaxis()->SetTitle("Talpha-TRF [ADC units]");
  ...
  talpha_proj->GetXaxis()->SetTitle("Talpha-TRF [ADC units]");
  talpha_proj->GetXaxis()->CenterTitle(true);
  talpha_proj->GetYaxis()->SetTitleOffset(1.5);
  gStyle->SetPalette(1);

  tproton_proj->GetXaxis()->SetTitle("Tproton-TRF [ADC units]");
