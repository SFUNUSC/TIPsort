...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_ECalABSuppLayerSum master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalABSuppLayerSum histograms for TIGRESS.\n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting ECalABSuppLayerSum histograms for TIGRESS clovers and cores based upon the cluster file: %s\n",name->fname.cluster_file);
