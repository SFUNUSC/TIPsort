...
int main(int argc, char *argv[])
{
  ...
  char mcaFile[132];
  int i,j;
  double N,Nb,Np,T,gatedT;
  double totN,totGatedT;
  ...
  totN=0.;
  totGatedT=0.;
  /* background subtraction (done using double precision for background) */
  /* loop over the rings */
  for(j=0;j<NRING;j++)
    {
      N=0.;
      ...
      for(i=0;i<S32K;i++)
	{
	  ...
	  T+=(double)tproj[j][i];
	  gatedT+=(double)gated[j][i];
	  //gatedT+=(double)gated[j+NRING][i];
	}
      ...
      for(i=(int)relow[j];i<=(int)rehigh[j];i++)
	{
      ...
      N=Np+Nb;
      totN+=N;
      printf("T %.2f gatedT %.2f Np %.2f Nb %.2f N %.2f foldCorr %f \n",T,gatedT,Np,Nb,N,gatedT/N);

      //trying to fix the fold issue
      double foldCorr=0.;
      foldCorr=gatedT/N;
      Np*=foldCorr;
      Nb*=foldCorr;
      ...
      for(i=0;i<S32K;i++)
      	{
      	  final[j][i] = (int)rint(dgated[j][i]-(Nb/T)*dtproj[j][i]-(Np/T)*dbg[j][i]);
      	  sbg[j][i]=(int)rint(dbg[j][i]);
      	  tbg[j][i]=(int)rint((Nb/T)*dtproj[j][i]+(Np/T)*dbg[j][i]);
      	}
    }
  /* end loop over rings */
  ...
  double r=0.;
  r=totGatedT/totN;
  printf("total gatedT %.0f total N %.0f r %.3f\n",totGatedT,totN,r);
