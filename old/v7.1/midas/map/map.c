...
void print_map(tmap* map)
{
  ...
  if((map->tigr_css_max-map->tigr_css_min)>0)
    {
      ...
      if(map->tigr_fit==1) printf("FIT       YES\n");
      else 	           printf("FIT       NO\n");
	...
	{
	  if(map->tigr_ge_map[i]!=0) printf(" %2d         %2d\n",i,map->tigr_ge_map[i]);
	}
      printf("=======    ==========================================\n");
    }
  ...
  if((map->grif_css_max-map->grif_css_min)>0)
    { 
      ...
      if(map->grif_fit==1) printf("FIT       YES\n");
      else                 printf("FIT       NO\n");
	...
	{
	  if(map->grif_ge_map[i]!=0) printf(" %2d         %2d\n",i,map->grif_ge_map[i]);
	}
      printf("=======    ==========================================\n");
    }
  ...
  if((map->csiarray_max-map->csiarray_min)>0)
    {
      ...
      if(map->csiarray_fit==1) printf("FIT       YES\n");
      else                     printf("FIT       NO\n");
      ...
      for(i=0;i<NCSI;i++)
	{
	  if(map->csiarray_map[i]!=0) printf(" %2d         %2d\n",i,map->csiarray_map[i]);
	}
      printf("=======    ==========================================\n");
    }

  if((map->pinarray_max-map->pinarray_min)>0)
    {
      printf("PINArray  CHANNEL\n");
      printf("MIN       %d\n",map->pinarray_min);
      printf("MAX       %d\n",map->pinarray_max);
      
      if(map->pinarray_fit==1) printf("FIT       YES\n");
      else                     printf("FIT       NO\n");
      ...
      printf("=======    ==========================================\n");
      printf("PINArray   GROUP2POSITION\n");
      printf("Group      Position\n");
      for(i=0;i<NCSI;i++)
	{
	  if(map->pinarray_map[i]!=0) printf(" %2d         %2d\n",i,map->pinarray_map[i]);
	}
      printf("=======    ==========================================\n");
    }
  
  if((map->tiffin_max-map->tiffin_min)>0)
    {
      printf("TIFFIN  CHANNEL\n");
      printf("MIN       %d\n",map->tiffin_min);
      printf("MAX       %d\n",map->tiffin_max);
      
      if(map->tiffin_fit==1) printf("FIT       YES\n");
      else                   printf("FIT       NO\n");
      ...
      printf("=======    ==========================================\n");
      printf("TIFFIN   GROUP2POSITION\n");
      printf("Group      Position\n");
      for(i=0;i<NTIFF;i++)
	{
	  if(map->tiffin_map[i]!=0) printf(" %2d         %2d\n",i,map->tiffin_map[i]);
	}
      printf("=======    ==========================================\n");
    }}
...
void read_map(char* name,tmap* map)
{
  ...
  else printf("TDAQ map read from file %s\n",name);
  ...
  //memset(map->pinarray_map,0,60*sizeof(int));
  //memset(map->tigr_ge_map,0,NPOSTIGR*sizeof(int));
  //memset(map->grif_ge_map,0,NPOSGRIF*sizeof(int));
	...
	if(strcmp(str2,"CHANNEL")==0)
	  {
	    ...
	    if(fscanf(inp,"%s %s",str1,str2)!=EOF)
              if(strcmp(str1,"KPAR")==0)
                map->pinarray_kpar=atof(str2);
            if(fscanf(inp,"%s %s",str1,str2)!=EOF)
              if(strcmp(str1,"DISP")==0)
                map->pinarray_disp=atof(str2);
	  }
     ...
     if(strcmp(str1,"TIFFIN")==0)
       if(strcmp(str2,"CHANNEL")==0)
	 {
	   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
	     if(strcmp(str1,"MIN")==0)
	       map->tiffin_min=atoi(str2);
	   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
	     if(strcmp(str1,"MAX")==0)
	       map->tiffin_max=atof(str2);
	   ...
	   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
	     if(strcmp(str1,"KPAR")==0)
	       map->tiffin_kpar=atof(str2);
	   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
	     if(strcmp(str1,"DISP")==0)
	       map->tiffin_disp=atof(str2);
	 }
	   ...
	   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
		 ...
		 while(take==1)
		   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
			 ...
			 map->tiffin_map[atoi(str1)]=atoi(str2);
		     }}
       ...
       if(strcmp(str2,"CHANNEL")==0)
	 {
	   ...
	   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
	     if(strcmp(str1,"KPAR")==0)
	       map->csiarray_kpar=atof(str2);
	   if(fscanf(inp,"%s %s",str1,str2)!=EOF)
	     if(strcmp(str1,"DISP")==0)
	       map->csiarray_disp=atof(str2);
...
void map_event(Tig10_event *ptr, short* waveform, raw_event *data,tmap* map,int turn)
{
	...
	if((pos>0) && (pos<NPOSTIGR))
	  {
		...
		if(waveform!=NULL)
		  if(map->tigr_fit==1)
		    {
		      //get_Tigress_t0(ptr->waveform_length,waveform,&data->tg.det[pos].ge[col].wfit[pos],&wpar);
		      //data->tg.det[pos].ge[col].t0[d]=wpar.t00;
		      take|=1;
		    }
		...
		if(take==1)
		  {
		    ...
		    //Repeat THP/Tfold in case a BGO, but not Ge fires. For timing!
		    if((data->tg.g.THP&(one<<id))==0) 
		      {
			data->tg.g.THP|=(one<<id);
			data->tg.g.Tfold++;
		    ...
		    //count clovers
		    id=pos-1;
		    //Repeat PosHP/Posfold in case a BGO, but not Ge fires.
		    if((data->tg.g.PosHP&(one<<id))==0)
		      {
			data->tg.g.PosHP|=(one<<id);
			data->tg.g.Posfold++;
  ...
  /*This is temporary. If you're aren't PJV, don't use it!!!*/
  if(ptr->channel==730) ptr->channel=722;
  if(ptr->channel==731) ptr->channel=723;


  //if(ptr->channel==720) ptr->channel=740;
  //else if(ptr->channel==730) ptr->channel=720;
  //else if(ptr->channel==731) ptr->channel=741;
  /*^^^^This is temporary. If you're aren't PJV, don't use it!!!^^^^*/
	...
	if((pos>0) && (pos<NPIN))
	  {
	      ...
	      if(ptr->charge!=0)
		{
		  data->pinarray.pin[pos].charge=
		    (ptr->charge/(map->pinarray_kpar/map->pinarray_disp));
		...
		if(waveform!=NULL)
		  if(map->pinarray_fit==1)
		    {
		      //get_PIN_t0(ptr->waveform_length,waveform,&data->pinarray.wfit[pos],&wpar);
		      //data->pinarray.t0[pos]=wpar.t00;
		    }}}}
  /*********************** stop PIN diode array ************************/
  /*********************** start TIFFIN ************************/
  if(ptr->channel>=map->tiffin_min)
    if(ptr->channel<=map->tiffin_max)
      {
	c=ptr->channel-map->tiffin_min;
	pos=map->tiffin_map[c];
	if((pos>0) && (pos<NTIFF))
	  {
	    take=0;
	    if((ptr->charge_flag==1) && (ptr->charge!=0))
	      {
		data->tiffin.chan[pos].charge=
		  (ptr->charge/(map->tiffin_kpar/map->tiffin_disp));
		data->tiffin.h.Efold++;
		data->tiffin.h.EHP|=(one<<pos);
		take|=1;
	      }
	    if((ptr->cfd_flag==1) && (ptr->cfd!=0))
	      {
		data->tiffin.chan[pos].cfd=ptr->cfd;
		data->tiffin.h.Tfold++;
		data->tiffin.h.THP|=(one<<pos);
		take|=1;
	      }
	    
	    if(take==1)
	      {
		data->tiffin.chan[pos].timestamp=ptr->timestamp;
		data->tiffin.chan[pos].timestamp_up=ptr->timestamp_up;
		data->tiffin.h.TSfold++;
		data->tiffin.h.TSHP|=(one<<pos);
		data->h.setupHP|=TIFFIN_BIT;
	      }//if(waveform!=NULL)
		//if(map->tiffin_fit==1)
		//{
		//get_PIN_t0(ptr->waveform_length,waveform,&wpar);
		//data->tiffin.t0[pos]=wpar.t00;
		//}
	  }}
  /*********************** stop TIFFIN array ************************/
  /*********************** start  CSI array ************************/
	...
	if((pos>0) && (pos<NCSI))
	  {
	      ...
	      if(ptr->charge!=0)
		{
		  data->csiarray.csi[pos].charge=
		    (ptr->charge/(map->csiarray_kpar/map->csiarray_disp));
		  ...
		  if(map->csiarray_fit==1)
		    {
		      ...
		      //get_CsI_t0(ptr->waveform_length,waveform,&data->csiarray.wfit[pos],&wpar);
  ...
  if(ptr->channel==map->rf)
      ...
      if(waveform!=NULL)
	{
	  if(get_sin_par(map->rf_period,ptr->waveform_length,waveform,&data->rf.sin)==1)
	    {
	      data->rf.sin.t0*=16; //in units of samples * 16 -> CFD units now.
