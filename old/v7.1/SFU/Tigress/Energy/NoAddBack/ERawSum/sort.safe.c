...
{
  FILE *output, *pcluster, *ClusterFile, *DataFile;
  ...
  if(argc!=2)
    {
      printf("Tigress_ERaw master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts summed raw energy histograms for Tigress cores and crystals from a cluster file\n");
