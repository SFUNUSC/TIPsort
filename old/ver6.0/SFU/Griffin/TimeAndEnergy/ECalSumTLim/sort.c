...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Griffin_ECalSumTLim master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalSumTLim energy histograms for GRIFFIN cores \n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting ECalSumTLim energy histograms for GRIFFIN cores based upon the cluster file: %s\n",name->fname.cluster_file);
  ...
  for(pos=1;pos<NPOSGRIF;pos++)
	...
	stop+=cal_par->gr.ceflag[pos][col];
      if(stop>0)
	{
	  sprintf(n,"pos%1d%1d_ECalSumTLim.mca",pos/10,pos-(pos/10)*10);
