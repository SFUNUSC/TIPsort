...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("Tigress_ECalABLayerSum master_file_name\n");
      exit(-1);
    }
  
  printf("Program sorts ECalABLayerSum histograms for TIGRESS.\n");
  ...
  if(name->flag.cluster_file==1)
    {
      printf("Sorting ECalABLayerSum histograms for TIGRESS clovers and cores based upon the cluster file: %s\n",name->fname.cluster_file);
