				  ...
				  if(((data->tg.h.BGOHP&(1<<(posBGO-1)))!=0) && (data->tg.det[posBGO].h.BGOfold>0))
					...
					for(supBGO=0;supBGO<NSUP;supBGO++)
					  ...
					  if(((data->tg.det[posBGO].bgo[colBGO].h.THP&(1<<supBGO))!=0)) //&& (take==0))
					    {
					      //printf("am i in here?\n");
					      //should there be a time check in here?
					      ...
					      //do I need the take stuff in this code? prevents double counting BGO for high fold.
					      //may negatively impact suppression in high fold events
					      //take=1; 
					    }
				}
			    }
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=5)
    {
      printf("TigressBGO_TTHP master_file_name N_sigma tigE_low tigE_high\n");
      exit(-1);
    }
