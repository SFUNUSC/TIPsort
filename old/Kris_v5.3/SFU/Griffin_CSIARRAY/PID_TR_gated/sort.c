	...
	if(type>=idmin)
	  if(type<=idmax)
	      ...
	      if(chi<=chimax)
		{
		  ...
		  r=100+s/f*100;
		  if(pGateFlag[pos]==1)
		    if(pGate[pos]->IsInside(t,r)) 
		      np++;
		  if(aGateFlag[pos]==1)
		    if(aGate[pos]->IsInside(t,r)) 
		      na++;
		}
      }
...
int main(int argc, char *argv[])
{
  ...
  if(argc!=6)
    {
      printf("Griffin__CSIARRAY_PIDTR_gated master_file_name idmin idmax chimin chimax\n");
      exit(-1);
    }
  ...
  if((out=fopen("ch.mca","w"))==NULL)
    {
      printf("ERROR!!! I cannot open the ch.mca file\n");
      exit(EXIT_FAILURE);
    }
