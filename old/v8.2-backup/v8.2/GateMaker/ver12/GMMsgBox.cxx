                       ...
                       Int_t text_align)
{
   ...
   fPlot = fOverlay = fDelete = fRename = fRetry = fIgnore = fCancel = fClose =
   ...
   buttons &= (kGMPlot | kGMOverlay | kGMDelete | kGMRename |
               kGMRetry | kGMIgnore | kGMCancel | kGMClose | kGMDismiss |
  ...
  if (buttons & kGMDelete) {
   ...
   if (buttons & kGMRetry) {
      fRetry = new TGTextButton(fButtonFrame, new TGHotString("&Retry"), kGMRetry);
