...
int main(int argc, char *argv[])
{
  ...
  if(argc!=2)
    {
      printf("PINArray_TRaw sfu_input_data\n");
      exit(-1);
    }
  
  h = new TH2D("TRaw","TRaw",S32K,0,S32K-1,NPIN+1,0,NPIN);
  h->Reset();
  
  printf("Program sorts TRaw histograms for the PINArray.\n");
  ...
  if((output=fopen("PINArray_TRaw.mca","w"))==NULL)
    {
  ...
  canvas = new TCanvas("TRaw","TRaw",10,10, 500, 300);
  gPad->SetLogz(1);
  gStyle->SetPalette(1);
  h->Draw("COLZ");
