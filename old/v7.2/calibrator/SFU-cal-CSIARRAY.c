...
void read_CSIARRAY_e_cal_par(CSIARRAY_calibration_parameters *CSIARRAY_cal_par, char *filename)
{
  ...
  printf("************\n");
  printf("************\n");
  printf("The CsI raw energy is now taken from the DAQ evaluation,\n");
  printf("rather than the waveform fits!!! Make a change in the calibrator\n");
  printf("if you'd rather have the waveform amplitude for energy evaluation!!!\n");
  printf("************\n");
  printf("************\n");
  printf("************\n");
  printf("ATTENTION!!!\n");
...
void calibrate_CSIARRAY(raw_event* rev, CSIARRAY_calibration_parameters *CSIARRAY_cal_par, cCSIARRAY *cp)
{
  ...
  /* 	      ren=rev->csiarray.wfit[pos].am[1]+ran; */
  ...
  /* then using DAQ energies */
  if(rev->csiarray.h.Efold>0)
    for(pos=1;pos<NCSI;pos++)
      if(CSIARRAY_cal_par->ceflag[pos]==1)
	if((rev->csiarray.h.EHP&(one<<pos))!=0)
	  if(rev->csiarray.csi[pos].charge>0)
  	    {
  	      ran=(double)rand()/(double)RAND_MAX-0.5;
	      ...
	      if((e>=0) && (e<4*S65K))
		{
	      ...
	      else if(e<0) printf("Warning! Calibrated CsI energy is less than 0!\n");
	      else         printf("Warning! Calibrated CsI energy is greater than 4*S23K! Raw energy was %f.\n",(ren-ran));
	    }
